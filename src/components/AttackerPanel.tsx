...
const AttackerSection: React.FC<AttackerSectionProps> = ({
  ...
  } else if ((moveName === "ふんか" || moveName === "しおふき" || moveName === "ドラゴンエナジー") && attacker.move) { // ★ 修正: ドラゴンエナジーを追加
  ...
  return (
    <div className={`bg-gray-800 rounded-lg p-4 mb-4 ${!attacker.isEnabled ? 'opacity-60' : ''}`}>
      ...
      <div className={`${!attacker.isEnabled ? 'pointer-events-none' : ''}`}>
        ...
        {showPhotonGeyserCategoryInfo && (<div className="mt-2 text-sm text-gray-300">フォトンゲイザー計算カテゴリ: <span className="font-semibold text-white">{attacker.photonGeyserDeterminedCategory === MoveCategory.Physical ? '特殊' : '物理'}</span></div>)}
