...
const DamageResult: React.FC<DamageResultProps> = ({
  ...
  const handleDoubleBattleChangeFromInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    setIsDoubleBattle(event.target.checked);
  };
  ...
  const getKOText = (damagesPerSingleHitDistribution: number[], hp: number, currentHitCount: number, showPercentage: boolean = true): string => {
    ...
    if (koChance < 0.01 && koChance > 0) return `乱数${minUsagesToKO}発 (<0.01%)`;
    if (koChance > 99.99 && koChance < 100) return `乱数${minUsagesToKO}発 (>99.99%)`;
    return `乱数${minUsagesToKO}発 (${formatPercentage(koChance)}%)`;
  };

  const getCombinedKORangeText = (combinedMinDamage: number, combinedMaxDamage: number, hp: number): string => {
    if (hp <= 0) return "確定1発";
  ...
  const disasterMap: { [key in keyof DisasterState]: string } = { sword: "わざわいのつるぎ", ball: "わざわいのたま", vessel: "わざわいのうつわ", talisman: "わざわいのおふだ" };
  const uniqueDoubleBattleId = `doubleBattle-${resultIdSuffix}`;
  const uniqueCriticalModeId = `criticalMode-${resultIdSuffix}`;

  const handleSaveLog = () => { if (onSaveLog) onSaveLog(); };
  const openModalAndSaveLog = () => { setIsModalOpen(true); handleSaveLog(); };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 z-[1000] flex items-center justify-center p-4">
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 relative">
        <div className="flex justify-between items-center mb-4">
            ...
            <button className="text-gray-400 hover:text-white" onClick={() => setIsModalOpen(false)} aria-label="閉じる"><X className="w-6 h-6" /></button>
        </div>
        <div className="mb-2">
        ...
        <div className="mb-2">
          ...
          <p className="text-white font-medium"><span className={getDamageColor(multiHitMinPercentage)}>{multiHitMinDamage}</span>{' - '}<span className={getDamageColor(multiHitMaxPercentage)}>{multiHitMaxDamage}</span><span className="text-sm text-gray-400 ml-2">({formatPercentage(multiHitMinPercentage)}% - {formatPercentage(multiHitMaxPercentage)}%) {' '}{getKOText(result.normalDamages, defenderHP, hitCount, true)}</span></p>
        </div>
        <div className="mb-4">
          <p className="text-sm text-red-400 mb-1">急所{hitCount > 1 && ` (${hitCount}回)`}</p>
          <p className="text-white font-medium"><span className={getDamageColor(multiHitCritMinPercentage)}>{multiHitCritMinDamage}</span>{' - '}<span className={getDamageColor(multiHitCritMaxPercentage)}>{multiHitCritMaxDamage}</span><span className="text-sm text-gray-400 ml-2">({formatPercentage(multiHitCritMinPercentage)}% - {formatPercentage(multiHitCritMaxPercentage)}%) {' '}{getKOText(result.criticalDamages, defenderHP, hitCount, true)}</span></p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="flex items-start mb-2">
                <img src={`/icon/${attackerDetails!.pokemonId.toString().padStart(3, '0')}.png`} alt={attackerDetails!.pokemonName} className="w-12 h-12 inline-block mr-2 border border-gray-700 rounded"/>
                <div className="flex flex-col space-y-1">{Array.isArray(attackerDetails!.displayTypes) && attackerDetails!.displayTypes.map((type, idx) => (type && <span key={`attacker-type-${idx}`} className="px-2 py-0.5 rounded text-xs font-medium text-white" style={{ backgroundColor: getTypeColorFromModal(type as PokemonType | 'stellar') }}>{getTypeNameJpFromModal(type as PokemonType | 'stellar')}</span>))}</div>
            </div>
            <ul className="space-y-1 text-sm text-gray-300">
              <li>技の威力: <span className="font-semibold text-white">{attackerDetails!.movePower}</span></li>
              {attackerDetails!.moveCategory && (<li>技カテゴリ: <span className="font-semibold text-white">{attackerDetails!.moveCategory === 'physical' ? '物理' : attackerDetails!.moveCategory === 'special' ? '特殊' : '変化'}</span></li>)}
          ...
          <div>
            <h3 className="text-lg font-medium text-blue-400 mb-3 border-b border-gray-700 pb-1">防御側: {defenderDetails!.pokemonName}</h3>
             <div className="flex items-start mb-2">
                <img src={`/icon/${defenderDetails!.pokemonId.toString().padStart(3, '0')}.png`} alt={defenderDetails!.pokemonName} className="w-12 h-12 inline-block mr-2 border border-gray-700 rounded"/>
                <div className="flex flex-col space-y-1">{Array.isArray(defenderDetails!.displayTypes) && defenderDetails!.displayTypes.map((type, idx) => (type && <span key={`defender-type-${idx}`} className="px-2 py-0.5 rounded text-xs font-medium text-white" style={{ backgroundColor: getTypeColorFromModal(type) }}>{getTypeNameJpFromModal(type)}</span>))}</div>
            </div>
            <ul className="space-y-1 text-sm text-gray-300">
        ...
        {(weather || field || (disasters && Object.values(disasters).some(d => d))) && (
          <div className="mt-6 pt-4 border-t border-gray-700">
            ...
            <ul className="space-y-1 text-sm text-gray-300">
              ...
              {disasters && Object.entries(disasters).map(([key, value]) => value && <li key={key}>災い: <span className="font-semibold text-red-400">{disasterMap[key as keyof DisasterState]}</span></li>)}
            </ul>
          </div>
        )}
      ...
      {shouldDisplayCombinedResultSection && combinedResult && (
        <div className="mb-2">
           <div className="flex justify-between items-center mb-1">
            ...
            <button onClick={onToggleShowIndividualAttackResults} className="text-gray-400 hover:text-white p-1 -m-1" aria-label={showIndividualAttackResults ? "個別の攻撃結果を非表示にする" : "個別の攻撃結果を表示する"}>{showIndividualAttackResults ? <ChevronUp size={20} /> : <ChevronDown size={20} />}</button>
          </div>
          <><p className="text-white font-medium mb-1 text-sm"><span className={getDamageColor(combinedResult.minPercentage)}>{combinedResult.minDamage}</span>{' - '}<span className={getDamageColor(combinedResult.maxPercentage)}>{combinedResult.maxDamage}</span><span className="text-xs text-gray-400 ml-1">({formatPercentage(combinedResult.minPercentage)}% - {formatPercentage(combinedResult.maxPercentage)}%)</span><span className="text-xs text-gray-300 ml-1 font-semibold">{getCombinedKORangeText(combinedResult.minDamage, combinedResult.maxDamage, defenderHP)}</span></p><div className="w-full h-3 bg-gray-700 rounded-full overflow-hidden mb-1"><div className="h-full relative"><div className={`absolute h-full ${getOriginalDamageBarColor(combinedResult.minPercentage)}`} style={{ width: `${Math.min(100, combinedResult.minPercentage)}%` }}/><div className={`absolute h-full ${getOriginalDamageBarColor(combinedResult.maxPercentage)} opacity-50`} style={{ width: `${Math.max(0, Math.min(100, combinedResult.maxPercentage) - Math.min(100, combinedResult.minPercentage))}%`, left: `${Math.min(100, combinedResult.minPercentage)}%` }}/></div></div><div className="flex justify-between text-xs text-gray-400"><span>0</span><span>{defenderHP}</span></div></>
        </div>
      )}
      {showIndividualAttackResults && result && (
        <div className={`pt-1 ${shouldDisplayCombinedResultSection ? 'border-t border-gray-700 mt-2 pt-2' : ''}`}>
          <div className="w-full h-3 bg-white rounded-full overflow-hidden mb-1">
            <div className="h-full relative">{actualRemainingHPMinPercentage > 0 && (<div className={`absolute top-0 left-0 h-full ${getHpBarColorByRemainingHp(actualRemainingHPMinPercentage)}`} style={{ width: `${actualRemainingHPMinPercentage}%` }}/>)}{actualRemainingHPMaxPercentage > actualRemainingHPMinPercentage && (<div className={`absolute top-0 h-full ${getHpRangeBarColorByRemainingHp(actualRemainingHPMinPercentage)}`} style={{ left: `${actualRemainingHPMinPercentage}%`, width: `${Math.max(0, actualRemainingHPMaxPercentage - actualRemainingHPMinPercentage)}%`, }}/>)}</div>
          </div>
          <div className="flex justify-between items-center">
            <div className="flex-grow">
              <div>
                {isCriticalModeActive ? (
                  <div>
                    <p className="text-sm text-red-400 mb-1">急所 {hitCount > 1 && ` (${hitCount}回)`} {' '}{getKOText(result.criticalDamages, defenderHP, hitCount, true)}</p>
                    <p className="text-white font-medium"><span className={getDamageColor(multiHitCritMinPercentage)}>{multiHitCritMinDamage}</span>{' - '}<span className={getDamageColor(multiHitCritMaxPercentage)}>{multiHitCritMaxDamage}</span><span className="text-sm text-white ml-2">({formatPercentage(multiHitCritMinPercentage)}% - {formatPercentage(multiHitCritMaxPercentage)}%)</span></p>
                  </div>
                ) : (
                  <div>
                    <p className="text-sm text-white mb-1">通常 {hitCount > 1 && ` (${hitCount}回)`} {' '}{getKOText(result.normalDamages, defenderHP, hitCount, true)}</p>
                    <p className="text-white font-medium"><span className={getDamageColor(multiHitMinPercentage)}>{multiHitMinDamage}</span>{' - '}<span className={getDamageColor(multiHitMaxPercentage)}>{multiHitMaxDamage}</span><span className="text-sm text-white ml-2">({formatPercentage(multiHitMinPercentage)}% - {formatPercentage(multiHitMaxPercentage)}%)</span></p>
                  </div>
                )}
              </div>
            </div>
            <div className="flex items-center space-x-2 shrink-0 ml-2">
              <div className="flex items-center">
                <input type="checkbox" id={uniqueDoubleBattleId} checked={isDoubleBattle} onChange={handleDoubleBattleChangeFromInput} className="w-3 h-3 md:w-4 md:h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-offset-gray-800 focus:ring-1"/>
          ...
          {isModalOpen && attackerDetails && defenderDetails && modalRoot && ReactDOM.createPortal(ModalComponent, modalRoot)}
        </div>
      )}
    </div>
  );
